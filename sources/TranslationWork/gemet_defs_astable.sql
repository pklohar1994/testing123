...
SELECT le.id_concept AS code, le.value AS english,
    ...
    ls.value AS lang2term, ds.value AS lang2def,
    "" AS translation, "" AS transldef
FROM property AS le
...
LEFT JOIN property AS ds on le.ns=ds.ns AND le.id_concept=ds.id_concept AND ds.langcode=@secondlang AND ds.name="definition"
WHERE le.ns=1 AND le.langcode="en" AND le.name="prefLabel"
ORDER BY le.id_concept + 0
