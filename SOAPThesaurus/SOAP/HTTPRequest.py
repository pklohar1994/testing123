...
# version 5a -- added SOAP support using SOAPpy
#               all modified or new lines are commented with soap - tjl
#
#  tjl - that's me: Tim Lynch
#
#  March 9, 2004
...
class HTTPRequest(BaseRequest):
    ...
    def processInputs(
        ...
        if not hasattr(fs,'list') or fs.list is None:
            ...
            # or, Hmmm, maybe it's SOAP - tjl
            if (fs.headers.has_key('content-type') and
                fs.headers['content-type'][0:8] == 'text/xml' and
                method == 'POST'):
                if environ.has_key('HTTP_SOAPACTION'):
                # this is a SOAP request - tjl
                # note to myself -
                #     processInputs is called by Publish.py - look for response.setBody
                #     to see where the actual SOAP body is composed
                #
                # note that we get kwargs back from parse_input and that we stuff
                # them into the REQUEST object

                    global soap
                    import soap
                    meth, self.args, self.kwargs = soap.parse_input(fs.value)
                    self.isSOAP = 1        # nofault check for SOAP invocation - tjl
